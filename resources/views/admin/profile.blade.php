...
<style>
    ...
    .page-heading p { font-size: 16px; color: #64748b; margin-top: 8px; line-height: 1.6; }
    ...
    .profile-role { font-size: 12px; color: #64748b; margin-top: 3px; }
    .profile-email { font-size: 13px; color: #00827F; margin-top: 8px; word-break: break-all; }
...
<div class="page-heading">
    <h2>My Profile</h2>
    <p>Manage your profile details and administrative preferences for CareConnect.</p>
</div>
...
<div class="profile-grid">
    ...
    <div class="profile-sidebar">
        <div class="profile-avatar-wrap">
            <div class="profile-avatar">
                @if($admin->profile_picture)
                    <img id="avatarPreview" src="{{ asset('storage/' . $admin->profile_picture) }}" alt="Profile">
                @else
                    <span id="avatarInitials">{{ strtoupper(substr($admin->name, 0, 2)) }}</span>
        ...
        <div class="profile-name" style="text-align: center;">{{ $admin->name }}</div>
        <div class="profile-role" style="text-align: center;">Administrator</div>
        <div class="profile-email" style="text-align: center; display: block; margin-bottom: 20px;">{{ $admin->email }}</div>

        <div class="profile-info">
            <div class="info-item">
                <div class="info-label">Phone</div>
                <div class="info-value">{{ $admin->phone ?? 'Not set' }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Member Since</div>
                <div class="info-value">{{ $admin->created_at ? $admin->created_at->format('d M Y') : 'N/A' }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="profile-content">
        ...
        <div class="tab-pane active" id="tab-personal">
            <div class="form-card">
                ...
                <form method="POST" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" id="personalDetailsForm">
                    @csrf
                    @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="phone" name="phone" class="form-input"
                           value="{{ old('phone', $admin->phone) }}" placeholder="012-3456789"
                           pattern="01\d-\d{7,8}" title="Please enter a valid phone number (10-11 digits)" required>
                </div>

                <div class="form-group">
                    <label class="form-label">Email Address</label>
                    <input type="email" class="form-input readonly" value="{{ $admin->email }}" readonly disabled>
                </div>
